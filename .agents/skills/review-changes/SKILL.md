...
- Match the requested review: staged changes, unstaged changes, selected files, or a branch comparison. Record the comparison and its base and target states; resolve commit or branch references to commit IDs. For mutable working or staged content, record the selected file versions or content hashes.
- Read changed files and supporting targets from the reviewed target state. For staged checks, use index contents such as `git show :path`; for commit checks, use the selected commit. Working-tree files, including unstaged fixes and untracked replacements, are not evidence of staged or committed correctness.
...
5. Assess whether existing checks cover the actual change. Use relevant low-impact local checks when permitted; distinguish checks run now from earlier reported results, and report failures or skipped checks. State which version each check examined. A working-tree check does not validate a different staged or committed version. Do not install dependencies or claim that documentation review proves runtime behavior.
...
State the reviewed scope and recorded comparison, then list findings by practical importance:
...
Before returning the report, confirm the selected inputs still match the recorded comparison. If they changed, repeat the affected checks or mark the findings as stale. Return the report and comparison identity to the calling agent or active conversation. Do not edit files, save or post the report, stage changes, commit, push, merge, or submit coursework through this skill. A separate authorized implementation task can address the findings.
